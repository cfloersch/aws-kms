...
public class KmsKeyStore extends KeyStoreSpi {
    ...
    @Override
    public Key engineGetKey(String alias, char[] chars)
        throws NoSuchAlgorithmException, UnrecoverableKeyException
    {
        String prefixedAlias = getPrefixedAlias(alias);
        AliasListEntry aliasListEntry = getAliases().stream()
                                            .filter(e -> e.aliasName().equals(prefixedAlias))
                                            .findAny().orElse(null);
        ...
        boolean rsa = describeKeyResponse.keyMetadata().signingAlgorithmsAsStrings()
                                                        .stream().anyMatch(a -> a.startsWith("RSA"));
        return rsa ? KmsRSAKeyFactory.getPrivateKey(describeKeyResponse.keyMetadata().keyId())
                    : KmsECKeyFactory.getPrivateKey(describeKeyResponse.keyMetadata().keyId());
    }
